...
struct PastInclinationView: View {
    ...
    var body: some View {
        ...
        List(Array(zip(tree.pastInclinations.reversed(), tree.pastInclinationsDate.reversed())), id: \.1){ height, date in
            HStack{
        ...
        VStack{
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(alignment: .bottom, spacing: 16) {
                    ForEach(Array(tree.pastInclinations.enumerated()), id: \.offset) { index, value in
                        VStack {
                            RoundedRectangle(cornerRadius: 5)
                                .fill(.accent)
                                .frame(
                                    width: barWidth,
                                    height: CGFloat(value / (tree.pastInclinations.max() ?? 1)) * (chartHeight - 50)
                                )
